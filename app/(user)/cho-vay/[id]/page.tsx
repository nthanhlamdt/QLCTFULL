...
export default function EnhancedLoanDetailPage() {
  ...
  if (!loan) {
    return <div className="flex items-center justify-center h-screen text-2xl font-semibold">Khoản vay không tồn tại</div>
  }
  ...
  const handleAddTransaction = (e: React.FormEvent) => {
    ...
    if (newTransaction.type === 'payment') {
      updatedLoan.remainingAmount -= transactionAmount
    } else {
      updatedLoan.remainingAmount += transactionAmount
      updatedLoan.amount += transactionAmount
    }

    updatedLoan.transactions.push({
      id: Date.now(),
    ...
    if (updatedLoan.remainingAmount <= 0) {
      updatedLoan.status = 'Đã trả'
    } else if (updatedLoan.remainingAmount < updatedLoan.amount) {
      updatedLoan.status = 'Đang trả'
    }
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-6">Chi tiết khoản cho vay</h1>

      <Card className="mb-8 rounded-lg overflow-hidden">
        <CardHeader className="bg-black text-white">
          <CardTitle className="text-2xl">{loan.borrower}</CardTitle>
          <CardDescription className="text-gray-300">Thông tin khoản vay</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="space-y-1">
              <p className="text-sm font-medium text-gray-500">Số tiền cho vay</p>
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-gray-500">Ngày cho vay</p>
              <p className="text-lg">{loan.date}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-medium text-gray-500">Ngày đáo hạn</p>
              <p className="text-lg">{loan.dueDate}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-medium text-gray-500">Trạng thái</p>
              <Badge className={cn("text-white", getStatusColor(loan.status))}>{loan.status}</Badge>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-4 sm:space-y-0 sm:space-x-2">
            ...
            <Select value={filterType} onValueChange={setFilterType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="borrow">Cho vay thêm</SelectItem>
              </SelectContent>
            </Select>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full z-10 sm:w-[180px] pl-3 text-left font-normal">
                  {filterDate ? (
                   <span
                  ...
                  <TableRow key={transaction.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center">
                        ...
                        {transaction.type === 'payment' ? 'Trả nợ' : 'Cho vay thêm'}
                      </div>
                    </TableCell>
